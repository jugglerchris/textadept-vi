...
test.key('r', 'x')
assertEq(colno(), 1)
assertEq(buffer:get_line(0), 'xne two three four five\n')
test.key('$', 'r', 'z')
assertEq(colno(), 22)
assertEq(buffer:get_line(0), 'xne two three four fivz\n')
test.key('G', '$', 'r', 'P')
assertEq(lineno(), 2) assertEq(colno(), 22)
assertEq(buffer:get_line(2), 'some miscellaneous texP')

-- Tests with repeats
test.key('2', 'G', '4', 'r', 'K')
assertEq(lineno(), 1) assertEq(colno(), 4)
assertEq(buffer:get_line(1), 'KKKKbee cee dee ee eff\n')
